...
export function Trash({ request, onError }: { request: Request; onError: (message: string) => void }) {
  ...
  async function load(cursor = '') { cursor ? setLoadingMore(true) : setLoading(true); try { const data = await request<{ items: TrashItem[]; next_cursor: string; retention_days: number }>(`/trash?page_size=100${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ''}`); setItems((current) => cursor ? [...current, ...data.items] : data.items); setNextCursor(data.next_cursor); setRetentionDays(data.retention_days) } catch (error) { onError(error instanceof Error ? error.message : '读取回收站失败') } finally { setLoading(false); setLoadingMore(false) } }
  useEffect(() => { void load() }, [])
