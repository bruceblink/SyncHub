...
import {
  ChevronRight, Clock3, Download, File, Folder, FolderPlus, History, LogIn, LogOut, MonitorCog, Search,
...
function App() {
  ...
  const [page, setPage] = useState<'files' | 'sync' | 'trash' | 'activity'>('files')
  ...
  useEffect(() => { if (token) { void request<StorageUsage>('/account/usage', { token }).then(setUsage).catch(() => {}) } }, [token, items])
  async function authenticatedRequest<T>(path: string, options: Omit<RequestOptions, 'token'> = {}) {
  ...
  async function search(query: string) {
    ...
    try {
      const data = await request<FileList>(`/files/search?q=${encodeURIComponent(trimmed)}&page_size=100`, { token })
  ...
  return <main className="app-shell" onClick={() => setMenuID(null)}><aside className="sidebar"><div className="sidebar-brand"><div className="brand-mark small">S</div><span>SyncHub</span></div><nav><button className={`nav-item ${page === 'files' ? 'active' : ''}`} onClick={() => setPage('files')}><Folder size={18} />文件</button><button className={`nav-item ${page === 'trash' ? 'active' : ''}`} onClick={() => setPage('trash')}><Trash2 size={18} />回收站</button><button className={`nav-item ${page === 'activity' ? 'active' : ''}`} onClick={() => setPage('activity')}><Clock3 size={18} />活动记录</button><button className={`nav-item ${page === 'sync' ? 'active' : ''}`} onClick={() => setPage('sync')}><MonitorCog size={18} />同步状态</button></nav>{usage && <div className="usage"><span>云端空间</span><strong>{formatSize(usage.bytes_used)}</strong><small>{usage.file_count} 个文件</small></div>}<div className="account"><span>{user?.email || '已登录用户'}</span><button title="退出登录" onClick={logout}><LogOut size={18} /></button></div></aside><section className="workspace">{error && <div className="notice"><span>{error}</span><button onClick={() => setError('')}><X size={16} /></button></div>}{page === 'sync' ? <SyncStatus request={authenticatedRequest} onError={setError} /> : page === 'trash' ? <Trash request={authenticatedRequest} onError={setError} /> : page === 'activity' ? <Activity request={authenticatedRequest} onError={setError} /> : <><header className="toolbar"><div><p className="eyebrow">云端文件</p><div className="breadcrumbs"><button onClick={() => goTo(-1)}>我的文件</button>{trail.map((entry, index) => <span key={entry.id}><ChevronRight size={16} /><button onClick={() => goTo(index)}>{entry.name}</button></span>)}</div></div><div className="actions"><label className="search-box"><Search size={17} /><input value={searchQuery} placeholder="搜索文件和文件夹" onChange={(event) => void search(event.target.value)} />{searchQuery && <button title="清除搜索" onClick={() => void search('')}><X size={15} /></button>}</label><button className="icon-button" title="新建文件夹" onClick={() => setModal({ type: 'folder' })}><FolderPlus size={19} /></button><label className="primary upload-button"><Upload size={18} />上传文件<input type="file" onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; try { await upload(file) } catch (error) { setError(errorMessage(error)) } finally { event.target.value = '' } }} /></label></div></header>{searchResults && <p className="search-summary">“{searchQuery.trim()}” 的搜索结果：{searchResults.length} 项</p>}<div className="file-area"><div className="list-head"><span>名称</span><span>大小</span><span>修改时间</span><span /></div>{loading && !searchResults ? <div className="empty">正在读取文件...</div> : visibleItems.length === 0 ? <div className="empty"><Folder size={36} /><strong>{searchResults ? '没有匹配的文件' : `${currentPath} 还是空的`}</strong><span>{searchResults ? '尝试使用其他关键词。' : '上传文件或新建文件夹开始管理云端内容。'}</span></div> : visibleItems.map((item) => <div className="file-row" key={item.id}><button className="file-name" onClick={() => item.node_type === 'directory' && (setSearchResults(null), setSearchQuery(''), navigate(item))}><span className={item.node_type === 'directory' ? 'folder-icon' : 'file-icon'}>{item.node_type === 'directory' ? <Folder size={21} fill="currentColor" /> : <File size={21} />}</span><span>{searchResults ? item.path : item.name}</span></button><span>{item.node_type === 'file' ? formatSize(item.size) : '--'}</span><span>{formatDate(item.updated_at)}</span><div className="row-menu"><button className="icon-button compact" title="更多操作" onClick={(event) => { event.stopPropagation(); setMenuID(menuID === item.id ? null : item.id) }}><MoreHorizontal size={19} /></button>{menuID === item.id && <div className="menu" onClick={(event) => event.stopPropagation()}>{item.node_type === 'file' && <><button onClick={() => { void download(item).catch((error: unknown) => setError(errorMessage(error))); setMenuID(null) }}><Download size={16} />下载</button><button onClick={() => { setVersionFile(item); setMenuID(null) }}><History size={16} />版本历史</button></>}<button onClick={() => { setModal({ type: 'rename', item }); setMenuID(null) }}><Pencil size={16} />重命名</button><button className="danger" onClick={() => { setModal({ type: 'delete', item }); setMenuID(null) }}><Trash2 size={16} />删除</button></div>}</div></div>)}</div></>}{modal && <Dialog modal={modal} onClose={() => setModal(null)} onConfirm={async (value) => { try { if (modal.type === 'folder') await createFolder(value); else if (modal.type === 'rename') await rename(modal.item, value); else await remove(modal.item); setModal(null) } catch (error) { setError(errorMessage(error)) } }} />}{versionFile && <VersionHistory fileID={versionFile.id} fileName={versionFile.name} request={authenticatedRequest} onClose={() => setVersionFile(null)} onUpdated={load} onError={setError} />}</section></main>
}
