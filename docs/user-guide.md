...
当前 MVP 暂不包含 WebDAV、S3/OSS/MinIO、团队空间、共享目录和 MySQL adapter。SQLite 仍保留给本地开发和 smoke test。
...
| `DATABASE_DRIVER` | `postgres`（部署文件） | 可省略，服务会从 `DATABASE_URL` 推断；本地 SQLite 可设为 `sqlite`。 |
| `DATABASE_URL` | 无（Compose/Fly 必填） | PostgreSQL 连接串；不要提交到 Git。 |
