...
本文面向 SyncHub MVP 用户，说明如何部署服务端、安装 CLI/Agent、初始化工作区，并完成日常同步、版本恢复、备份和排查。

当前 MVP 的推荐形态是：

- 服务端：Linux 服务器 + Docker 镜像 + SQLite + 本地文件存储。
- 客户端：`synchub-cli` 执行登录、初始化和手动同步。
- 后台同步：`synchub-agent` 周期执行同步，可选监听本地变更。

当前 MVP 暂不包含 WebDAV、S3/OSS/MinIO、团队空间、共享目录和生产级 PostgreSQL/MySQL adapter。

下面示例以 `0.1.0` 为版本号；实际使用时请替换为当前最新 Release 版本。
...
curl -L -o docker-compose.release.yml \
  https://github.com/bruceblink/SyncHub/releases/download/v0.1.0/docker-compose.release.yml
```
...
```bash
mkdir -p ~/tmp/synchub-0.1.0
tar -xzf synchub-0.1.0-linux-amd64.tar.gz -C ~/tmp/synchub-0.1.0
sudo install -m 0755 ~/tmp/synchub-0.1.0/synchub-cli /usr/local/bin/synchub-cli
sudo install -m 0755 ~/tmp/synchub-0.1.0/synchub-agent /usr/local/bin/synchub-agent
synchub-cli version
...
Expand-Archive .\synchub-0.1.0-windows-amd64.zip -DestinationPath $installDir -Force
...
下文命令默认使用已安装的 `synchub-cli` 和 `synchub-agent`。如果从源码运行，把命令前缀替换为 `go run .\cmd\synchub-cli` 或 `go run .\cmd\synchub-agent`。
...
1. 安装 `synchub-cli` 和 `synchub-agent`。
2. 登录同一个 SyncHub 账号。
...
Agent 是对 `synchub-cli sync once` 的循环封装，适合日常持续同步。

执行一次 Agent 同步：

```powershell
synchub-agent --path $workspace --once
```

预览一次 Agent 同步：

```powershell
synchub-agent --path $workspace --once --dry-run
```

持续同步，每 30 秒执行一次：

```powershell
synchub-agent --path $workspace --interval 30s --device-name laptop --platform windows
```

监听本地变化，有变化时尽快触发同步：

```powershell
synchub-agent --path $workspace --watch --watch-interval 1s --interval 30s
```

查看 Agent 状态：

```powershell
synchub-agent --path $workspace --status
synchub-agent --path $workspace --status --json
```

暂停和恢复：

```powershell
synchub-agent --path $workspace --pause
synchub-agent --path $workspace --resume
```

清理 Agent 状态文件：

```powershell
synchub-agent --path $workspace --reset-state
```

Agent 状态默认写入：

```text
<workspace>/.synchub/agent-state.json
<workspace>/.synchub/agent-control.json
```
...
升级 CLI/Agent 时，下载新版本 Release archive，替换原来的 `synchub-cli` 和 `synchub-agent` 即可。
...
```powershell
$env:SYNCHUB_VERSION = "0.1.0"
$env:SYNCHUB_IMAGE = "synchub:0.1.0"
...
```powershell
synchub-agent --path $workspace --once
synchub-agent --path $workspace --watch --interval 30s
synchub-agent --path $workspace --status
synchub-agent --path $workspace --pause
synchub-agent --path $workspace --resume
```
