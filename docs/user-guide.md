...
- 服务端：Linux 服务器 + Docker 镜像 + PostgreSQL + 本地文件存储。
- 客户端：只需要安装 `synchub-cli`。
- 后台同步：`synchub-cli sync daemon` 默认监听本地变更，并按周期兜底同步。

当前 MVP 暂不包含 WebDAV、S3/OSS/MinIO、团队空间、共享目录和 MySQL adapter。服务端元数据统一使用 PostgreSQL。
...
docker run -d --name synchub-api \
  ...
  -e DATABASE_URL='postgresql://user:password@host:5432/synchub?sslmode=require' \
  -v synchub-data:/data \
...
- 数据库：外部 PostgreSQL，由 `DATABASE_URL` 指定；服务启动时会自动执行内置 migration。
- 文件对象：`/data/storage`
...
- `DATABASE_URL` 指向 PostgreSQL，例如 Neon 或其他托管 PostgreSQL。
- `/data/storage` 保存文件对象。
- `JWT_SECRET` 和 `DATABASE_URL` 使用 Fly secrets 设置，不写入 `fly.toml`。
- 不要把 Machine 数量扩到 2 个或更多；Fly Volume 不会自动复制，当前本地文件存储仍适合单实例部署。
...
fly secrets set --app $env:FLY_APP DATABASE_URL="postgresql://user:password@host:5432/synchub?sslmode=require"
```
...
| `DATABASE_DRIVER` | `postgres` | 兼容部署变量；服务端固定使用 PostgreSQL。 |
| `DATABASE_URL` | 无（所有环境必填） | PostgreSQL 连接串；不要提交到 Git。 |
| `DATABASE_SCHEMA` | 无 | 可选。测试或多环境共用同一数据库时指定 PostgreSQL schema。 |
| `STORAGE_BACKEND` | `local` | MVP 推荐保持默认。 |
| `LOCAL_STORAGE_ROOT` | `/data/storage` | 容器内文件对象存储路径。 |
| `STORAGE_QUOTA_BYTES` | `0` | 每个用户可用的逻辑文件容量；设为 `0` 表示不限额。 |
| `TRASH_RETENTION_DAYS` | `30` | 回收站自动保留天数；设为 `0` 可关闭自动清理。 |
| `OBJECT_GC_INTERVAL_SECONDS` | `3600` | 孤立存储对象的后台回收间隔；删除候选会先延迟并复查引用。 |
...
管理页面的“回收站”可恢复已删除文件和目录，也可在二次确认后彻底删除。页面会显示服务端配置的保留天数，超过保留期的顶层项目及目录子树由后台任务自动清理。彻底删除会永久移除该项目、目录子树及其版本历史，无法撤销；底层不可变对象由服务端后续的安全垃圾回收流程处理。
...
Compose 部署默认使用 `synchub-data` volume 保存本地文件对象；PostgreSQL 需要使用数据库服务商提供的备份或 `pg_dump` 单独备份。备份本地文件对象前建议停止写入或短暂停服：
...
恢复时先停服，再把文件对象备份解压回 volume，并把 PostgreSQL 恢复到时间点接近的备份。恢复前请确认目标 volume 可以被覆盖。

### 12.2 本地开发数据

本地开发同样使用 PostgreSQL。元数据通过 PostgreSQL 提供方的 dump、快照或 PITR 能力备份；文件对象位于 `LOCAL_STORAGE_ROOT`，应在停止 API 写入后单独备份。恢复时要选择时间点接近的数据库和对象备份，避免元数据与对象内容不一致。
...
如果有可用于测试的 PostgreSQL 连接串，可以运行完整 API/CLI smoke；脚本会自动创建并清理临时 schema：
