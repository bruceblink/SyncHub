...
这是 SyncHub 当前最合适的选择：项目核心是文件同步、流式 IO、版本一致性、后台任务和长期可维护的多端客户端，而不是普通 CRUD API。Rust + Axum 能让服务端、同步核心、CLI、Agent 和未来 Tauri GUI 共享领域模型与同步逻辑，降低长期维护成本。

Go + Gin 不作为主栈。Gin 的开发速度和部署简洁性很好，但如果 SyncHub 的 Agent、CLI 和 GUI 核心仍然倾向 Rust，服务端再使用 Go 会过早引入双语言成本。Go 可以保留为未来运维工具或边缘组件选项。
