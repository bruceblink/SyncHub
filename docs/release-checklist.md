...
PostgreSQL repository and API behavior are covered by `go test ./...`; integration tests use `DATABASE_URL` when a disposable database is available.
...
The script writes API server archives, deployment files, and `SHA256SUMS.txt` under `dist\synchub-0.2.0`. The desktop client is released from the companion `synchub-desktop` repository and is not bundled into the server archive.
...
Review [docs/releases/v0.2.0.md](releases/v0.2.0.md) before tagging.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\test-docker-compose.ps1 -Version 0.2.0
```
...
The worktree should be clean before tagging. Use `git tag --no-sign v0.2.0` only after the MVP check, Docker image smoke, and auxiliary artifact checks pass.
