...
SyncHub 采用单体优先架构。当前 MVP 不拆分独立服务进程，也不为未来微服务提前铺接口；只有当 SQLite + Local FS 的个人同步闭环稳定后，才评估是否拆分 worker 或 adapter。
...
- Metadata Store: 当前使用 SQLite，保存用户、设备、文件元数据、chunk、版本、变更日志。
...
cmd/
  ...
  synchub-cli/        # CLI and daemon-style sync commands
internal/
...
CLI / API Client
  ...
  -> SQLite metadata database + Local FS object storage
```
...
1. CLI daemon 上报 device_id、last_seen_change_id、本地文件摘要。
2. 服务端查询用户变更日志并与客户端摘要做 diff。
3. 服务端返回需要 pull、push、delete、conflict 的操作列表。
4. CLI daemon 执行上传 / 下载后提交同步结果。
...
- Phase 2: 增加 CLI daemon、文件监听和同步引擎。
- Phase 3: 补齐基础版本历史、恢复、pin 和最小版本清理。
- Later: WebDAV、S3-compatible storage、PostgreSQL / MySQL、Redis 队列和独立 worker，只有在 MVP 稳定后再评估。
