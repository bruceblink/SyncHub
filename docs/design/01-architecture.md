...
SyncHub 采用模块化单体优先、可演进微服务的架构。Phase 1 不拆分独立服务进程，而是在一个 Gin API server 中保持清晰 package 边界；当同步任务、存储任务或 WebDAV 适配出现独立扩缩容需求时，再拆成独立 worker 或服务。
...
- Storage Module: 统一 Storage interface，首期实现 Local FS，后续实现 S3-compatible storage。
- Metadata Store: 开发默认使用 SQLite，保存用户、设备、文件元数据、chunk、版本、变更日志；后续通过 repository 适配 PostgreSQL / MySQL。
...
cmd/
  ...
  synchub-agent/      # later phase agent entry
  synchub-cli/        # later phase CLI entry
internal/
  ...
  db/                 # SQLite bootstrap, repository adapters, transactions
  sync/               # sync planner, conflict detection, change log
  worker/             # background jobs
pkg/
  client/             # optional public API client for external integrations
migrations/
```
...
- Phase 1: 单进程 Gin API + SQLite + Local FS。
- Phase 1.x: 增加 PostgreSQL / MySQL repository adapter 和独立 migration 流程。
