...
func TestRunSyncDevicesCanOutputJSON(t *testing.T) {
	root := t.TempDir()
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet || r.URL.Path != "/api/v1/devices" {
			t.Fatalf("request = %s %s", r.Method, r.URL.Path)
	...
	if err != nil {
		t.Fatalf("sync devices json: %v", err)
	}
	if strings.Contains(stdout.String(), "devices:") {
		t.Fatalf("json output includes text devices output: %s", stdout.String())
	}

	var snapshot syncDevicesSnapshot
	...
	if !snapshot.Items[0].Current || snapshot.Items[0].ID != "dev_1" || snapshot.Items[0].LastAppliedChangeID != 9 || snapshot.Items[0].LastSeenAt == nil {
		t.Fatalf("first item = %#v", snapshot.Items[0])
	}
	if snapshot.Items[1].Current || snapshot.Items[1].ID != "dev_2" || snapshot.Items[1].LastSeenAt != nil {
