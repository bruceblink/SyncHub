...
func TestRunSyncPushKeepsDeletedManifestFileOnConflict(t *testing.T) {
	root := t.TempDir()
	remoteVersion := int64(3)
	deleted := false
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/v1/files/by-path":
			...
			_, _ = w.Write([]byte(`{"code":0,"message":"ok","data":{"id":"file_1","name":"remove.txt","path":"/workspace/remove.txt","node_type":"file","version":4}}`))
		case r.Method == http.MethodDelete && r.URL.Path == "/api/v1/files/file_1":
			var req struct {
				BaseVersion *int64 `json:"base_version"`
			}
			...
			if req.BaseVersion == nil || *req.BaseVersion != remoteVersion {
				t.Fatalf("base version = %#v", req.BaseVersion)
			}
			deleted = true
	...
	if len(updatedManifest.Items) != 1 || updatedManifest.Items[0].Path != "/workspace/remove.txt" || updatedManifest.Items[0].RemoteVersion == nil || *updatedManifest.Items[0].RemoteVersion != remoteVersion {
