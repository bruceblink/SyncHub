...
func TestRunFileMoveCanOutputJSON(t *testing.T) {
	root := t.TempDir()
	moved := false
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if got := r.Header.Get("Authorization"); got != "Bearer access" {
	...
	if err != nil {
		t.Fatalf("file move json: %v", err)
	}
	if !moved {
		t.Fatal("move endpoint was not called")
	}
	if strings.Contains(stdout.String(), "moved:") {
		t.Fatalf("json output includes text move output: %s", stdout.String())
	}
	var snapshot fileMutationSnapshot
	if err := json.Unmarshal(stdout.Bytes(), &snapshot); err != nil {
		t.Fatalf("decode move json: %v\n%s", err, stdout.String())
	}
	if snapshot.Action != "move" || snapshot.File.Path != "/workspace/docs/readme.txt" || snapshot.File.Version != 3 {
		t.Fatalf("snapshot = %#v", snapshot)
	}
	if snapshot.Source == nil || snapshot.Source.Path != "/workspace/readme.txt" || snapshot.Source.Version != 2 {
